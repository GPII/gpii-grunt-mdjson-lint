...
fluid.registerNamespace("gpii.tests.mdjsonLinter.extractPosition");

gpii.tests.mdjsonLinter.extractPosition.runAllTests = function (that) {
    fluid.each(that.options.testDefs, gpii.tests.mdjsonLinter.extractPosition.runSingleTest);
};

gpii.tests.mdjsonLinter.extractPosition.runSingleTest = function (testDef) {
    jqUnit.test(testDef.message, function () {
        var posDef = gpii.mdjsonLinter.extractPosition(testDef.errorString, testDef.originalMaterial);
        jqUnit.assertDeepEq("The output should be as expected...", testDef.expected, posDef);
    });
};

fluid.defaults("gpii.tests.mdjsonLinter.extractPosition.testRunner", {
    ...
    listeners: {
        ...
        "onCreate.runTests": {
            funcName: "gpii.tests.mdjsonLinter.extractPosition.runAllTests",
            args:     ["{that}"]
        }
    }
});

gpii.tests.mdjsonLinter.extractPosition.testRunner();
